...
export interface EntryFormProps {
  initialData?: IEntry;
  onSubmit: (entry: INewEntry) => void;
  submitText: string;
}

export const EntryForm: React.FC<EntryFormProps> = ({ initialData, onSubmit, submitText }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const submitFormWithData = async (data: INewEntry) => {
    ...
    // Create the entry object with base64 image
    const entry: INewEntry = {
      name: data.name,
    ...
    // Call the parent component's onSubmit with the entry object
    onSubmit(entry);
  };
  ...
  return (
    ...
    >
      ...
      <Input
        {...register("name", {
          required: "Le nom de la region est requis",
          minLength: {
            value: 3,
            message: "Le nom de la region doit contenir au moins 3 caractères",
          },
        })}
        ...
        radius="sm"
        placeholder="Entrez le nom de la region..."
        classNames={{
          base: "w-full",
          input: "text-sm",
      ...
      <div className="w-full">
        ...
        >
          <input
            ...
            //     console.log("Image data: ", e.target.value);
            //   }
            // }}
          />
